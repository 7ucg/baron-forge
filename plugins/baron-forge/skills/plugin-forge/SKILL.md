...
- **Framework plugin** (React, Vue, etc.): `.claude-plugin/plugin.json` + `skills/<name>/SKILL.md` + `references/` + `commands/prime/`
- **Utility plugin** (tools/commands only): `.claude-plugin/plugin.json` + `commands/` (no skills needed)
- **Domain plugin** (knowledge base): `.claude-plugin/plugin.json` + `skills/<name>/SKILL.md` + `references/` + `scripts/`

→ ref: `references/plugin-structure.md` for full directory layouts and manifest schemas.
